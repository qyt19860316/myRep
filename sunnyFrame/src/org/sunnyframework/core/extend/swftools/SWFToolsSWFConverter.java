...
import org.sunnyframework.core.util.PinyinUtil;
...
public class SWFToolsSWFConverter implements SWFConverter {
   ...
   private  boolean isWindowsSystem() {
	...
	public void convert2SWF(String inputFile, String swfFile, String extend) {
		...
		if (!pdfFile.exists()) {
			 org.sunnyframework.core.util.LogUtil.info("PDF文件不存在！");
			return;
		}
	
		if (outFile.exists()) {
			 org.sunnyframework.core.util.LogUtil.info("SWF文件已存在！");
			return;
		}
		try {
			// 开始转换文档
...
//	        					+ "\" -o " + swfFile + " -s languagedir=D:\\xpdf-chinese-simplified -T 9 -f";
			...
			try {
				process.waitFor();
				org.sunnyframework.core.util.LogUtil.info("时间-------"+process.waitFor());
